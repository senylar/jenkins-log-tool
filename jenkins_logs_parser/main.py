...
class JenkinsClient:
    ...
    def _job_url(self, job_name: str) -> str:
        ...
        """
        if job_name.startswith("https://") or job_name.startswith("http://"):
...
def save_logs_to_file(logs, job_name, base_path_str):
    ...
    log_file_path = log_dir / f"{'_'.join(job_parts[1:])}.log"
...
def get_job_name_from_url(url : str):
...
def main():
    ...
    if not args.job_name.startswith('http'):
        parser.error("Требуется указать URL джобы")
    ...
    if args.lnav:
        ...
        if log_path == 'empty':
            ...
            return
        filename = args.job_name
        if args.job_name.startswith('http'):
            filename = get_job_name_from_url(args.job_name)
        save_logs_to_file(logs, filename, log_path)
